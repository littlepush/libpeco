...
int main( int argc, char* argv[] ) {
    ...
    this_loop.do_job([]() {
        std::string _host = "https://rtcio-test.bytedance.com/api/ping";
        std::vector< double > _times;
        for ( int i = 0; i < 100; ++i ) {
            this_loop.do_job([&_host, i, &_times]() {
                parent_task::guard _pg;
                this_task::begin_tick();
                auto _req = http_request::fromURL(_host);
                auto _resp = http_connection::send_request(_req, peer_t("111.6.226.229:443"));
                double _time = this_task::tick();
                _times.push_back(_time);
                std::cout << "Request test: " << i << ", time used: " << _time << "ms, code: " << _resp.status_code << std::endl;
            });
        }
        while ( _times.size() != 100 ) {
            this_task::holding();
        }
        std::sort(_times.begin(), _times.end(), std::less<double>());

        auto _data_filter = [&_times]( int count ) {
            double _pc = 0;
            for ( int i = 0; i < count; ++i ) {
                _pc += _times[i];
            }
            std::cout << "top " << count << "% avg: " << (_pc / count) << "ms" << std::endl;
        };
        _data_filter(50);
        _data_filter(75);
        _data_filter(90);
